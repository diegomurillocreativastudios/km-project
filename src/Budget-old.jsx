...
// Move TransactionRow component definition to the top level
...
// Move renderReconciliation to the top level
const renderReconciliation = (month, data, formatCurrency, endingBalances, handleEndingBalanceChange) => {
  ...
  const lastDayTransactions = monthTransactions.filter(t => t.date === lastDate)
    .sort((a, b) => a.balance - b.balance);

  const startingBalance = firstTransaction.balance - firstTransaction.amount;
  const endingBalance = lastDayTransactions[lastDayTransactions.length - 1].balance;
  const expectedEndingBalance = startingBalance + data.income - data.expenses;
  const discrepancy = endingBalance - expectedEndingBalance;

  return (
    <div className="mt-8 bg-white p-6 rounded-lg border border-blue-200">
      <h4 className="text-lg font-medium mb-4 text-gray-800">Balance Reconciliation</h4>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-gray-600">Starting Balance:</span>
            <span className="text-sm font-bold">{formatCurrency(startingBalance)}</span>
          </div>
          <div className="flex justify-between items-center text-green-600">
            <span className="text-sm font-medium">Total Income:</span>
            <span className="text-sm font-bold">+{formatCurrency(data.income)}</span>
          </div>
          <div className="flex justify-between items-center text-red-600">
            <span className="text-sm font-medium">Total Expenses:</span>
            <span className="text-sm font-bold">-{formatCurrency(data.expenses)}</span>
          </div>
          <div className="border-t pt-4 flex justify-between items-center">
            <span className="text-sm font-medium">Expected Ending Balance:</span>
            <span className="text-sm font-bold">{formatCurrency(expectedEndingBalance)}</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => handleEndingBalanceChange(month, e.target.value)}
                className="w-32 p-1 border rounded text-right"
                step="0.01"
                placeholder="Enter balance"
              />
              <span className="text-sm font-bold">
                {endingBalances[month] ? formatCurrency(endingBalances[month]) : '---'}
              </span>
            </div>
          </div>
          <div className={`flex justify-between items-center ${
            Math.abs(endingBalances[month] - expectedEndingBalance) > 0.01 ? 'text-red-600' : 'text-green-600'
          }`}>
          ...
          {endingBalances[month] && Math.abs(endingBalances[month] - expectedEndingBalance) > 0.01 && (
            <div className="mt-4 p-4 bg-yellow-50 rounded-lg">
              <h5 className="text-sm font-medium text-yellow-800 mb-2">Possible Causes:</h5>
...
const ImprovedMultiStatementBudget = () => {
  ...
  // File upload handling (original file processing)
  ...
  // New: Wrap file upload to check for metadata and set ending balances before processing CSVs.
  const handleFileUploadWithMetadata = (event) => {
    // Read metadata from the first file if available
    const files = event.target.files;
    ...
    // Then call the original file processing function
    ...
    return data
      .map((item, index) => {
        ...
        if (isCreditCardStatement) {
          ...
          // Ensure credit card transactions are properly categorized
          ...
          // Credit card transactions: debits are negative, credits are positive
  ...
  // Old helper functions (preserved for backward compatibility)
  ...
  const renderCurrent = () => {
    ...
    transactions.forEach(t => {
      ...
      if (!excludedTransactions.includes(t.id)) {
        if (t.amount > 0) {
          monthlyData[monthYear].income += t.amount;
          monthlyData[monthYear].categories.income[t.category] = 
            (monthlyData[monthYear].categories.income[t.category] || 0) + t.amount;
        } else {
          monthlyData[monthYear].expenses += Math.abs(t.amount);
          monthlyData[monthYear].categories.expense[t.category] = 
            (monthlyData[monthYear].categories.expense[t.category] || 0) + Math.abs(t.amount);
        }
    ...
    // Inside renderCurrent function, update renderCategoryBreakdown:

const renderCategoryBreakdown = (month) => {
  if (!month || !monthlyData[month]) return null;
  const data = monthlyData[month];

  // Separate banking and credit card transactions
  const bankingTransactions = data.transactions.filter(t => !t.category.toLowerCase().includes('credit card'));
  const creditTransactions = data.transactions.filter(t => t.category.toLowerCase().includes('credit card'));

  // Calculate banking totals
  const bankingStartBalance = bankingTransactions
    .filter(t => t.balance !== undefined)
    .sort((a, b) => new Date(a.date) - new Date(b.date))[0]?.balance || 0;

  const bankingEndBalance = bankingTransactions
    .filter(t => t.balance !== undefined)
    .sort((a, b) => new Date(b.date) - new Date(a.date))[0]?.balance || 0;

  // Calculate banking income (including MISC_CREDIT)
  const bankingIncome = bankingTransactions
    .filter(t => t.amount > 0 || t.category === 'MISC_CREDIT')
    .reduce((sum, t) => sum + Math.abs(t.amount), 0);

  // Calculate banking expenses (excluding MISC_CREDIT)
  const bankingExpenses = bankingTransactions
    .filter(t => t.amount < 0 && t.category !== 'MISC_CREDIT')
    .reduce((sum, t) => sum + Math.abs(t.amount), 0);

  // Group banking transactions by category for income (including MISC_CREDIT)
  const bankingIncomeByCategory = {};
  bankingTransactions.forEach(t => {
    if (t.amount > 0 || t.category === 'MISC_CREDIT') {
      bankingIncomeByCategory[t.category] = (bankingIncomeByCategory[t.category] || 0) + Math.abs(t.amount);
    }
  });

  // Group banking transactions by category for expenses (excluding MISC_CREDIT)
  const bankingExpensesByCategory = {};
  bankingTransactions.forEach(t => {
    if (t.amount < 0 && t.category !== 'MISC_CREDIT') {
      bankingExpensesByCategory[t.category] = (bankingExpensesByCategory[t.category] || 0) + Math.abs(t.amount);
    }
  });
  ...
  // Group credit card transactions by category
  const creditExpensesByCategory = {};
  creditTransactions
    .filter(t => t.amount < 0)
    .forEach(t => {
      creditExpensesByCategory[t.category] = (creditExpensesByCategory[t.category] || 0) + Math.abs(t.amount);
    });
  ...
  return (
    <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="bg-white p-4 rounded-lg border border-blue-200">
        <h4 className="text-lg font-medium mb-4 text-gray-800">Banking Summary</h4>
        <div className="space-y-4">
          {/* Starting Balance */}
          <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
            <span className="text-sm font-medium text-gray-700">Starting Balance</span>
            <span className="text-sm font-bold">{formatCurrency(bankingStartBalance)}</span>
          </div>

          {/* Banking Income */}
          <div className="border-t pt-4">
            <h5 className="text-sm font-medium text-gray-600 mb-2">Income Sources</h5>
              ...
              {Object.entries(bankingIncomeByCategory)
                ...
                .map(([category, amount]) => (
                  <div key={category} className="flex justify-between items-center">
                    ...
                    <span className="text-sm text-green-600">{formatCurrency(amount)}</span>
                  </div>
                ))}
            </div>
            <div className="mt-2 pt-2 border-t">
              <div className="flex justify-between">
                <span className="text-sm font-medium">Total Income</span>
                <span className="text-sm font-bold text-green-600">{formatCurrency(bankingIncome)}</span>
              </div>
            </div>
          </div>
          ...
          {/* Banking Expenses */}
          <div className="border-t pt-4">
            <h5 className="text-sm font-medium text-gray-600 mb-2">Banking Expenses</h5>
            <div className="space-y-2">
            ...
            <div className="mt-2 pt-2 border-t">
              <div className="flex justify-between">
                <span className="text-sm font-medium">Total Banking Expenses</span>
                <span className="text-sm font-bold text-red-600">-{formatCurrency(bankingExpenses)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="border-t pt-4">
            <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
              <span className="text-sm font-medium text-gray-700">Calculated Ending Balance</span>
              <span className="text-sm font-bold">{formatCurrency(bankingEndBalance)}</span>
            </div>
          </div>

          {/* Actual Ending Balance Input */}
          <div className="border-t pt-4">
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium text-gray-700">Actual Ending Balance</span>
              <div className="flex items-center space-x-2">
                <input
                  type="number"
                  value={endingBalances[month] || ''}
      ...
      <div className="bg-white p-4 rounded-lg border border-red-200">
        <h4 className="text-lg font-medium mb-4 text-gray-800">Credit Card Expenses</h4>
        <div className="space-y-3">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex justify-between text-sm">
              <span className="font-medium text-gray-700">Total Credit Card Expenses</span>
    ...
    return (
      <div className="p-6 bg-white rounded-lg shadow">
          ...
          <table className="min-w-full mb-6">
            <thead>
              <tr className="border-b">
                ...
                <th className="py-2 text-right text-sm font-medium">Transaction Count</th>
              </tr>
            </thead>
            <tbody>
              {months.map(month => {
                ...
                return (
                  <tr 
                    key={month} 
                    className={`border-b hover:bg-gray-50 cursor-pointer ${selectedMonth === month ? 'bg-blue-50' : ''}`}
                  ...
                  >
                    ...
                    <td className={`py-2 text-right text-sm font-medium ${net >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {formatCurrency(net)}
                    </td>
            ...
            <tfoot className="bg-gray-50 font-medium">
              <tr>
                ...
                <td className="py-2 text-right text-sm text-green-600">
                  {formatCurrency(months.reduce((sum, month) => sum + monthlyData[month].income, 0))}
                </td>
                <td className="py-2 text-right text-sm text-red-600">
                  -{formatCurrency(months.reduce((sum, month) => sum + monthlyData[month].expenses, 0))}
                </td>
                <td className="py-2 text-right text-sm">
                  {formatCurrency(months.reduce((sum, month) => 
                    sum + (monthlyData[month].income - monthlyData[month].expenses), 0))}
                </td>
                <td className="py-2 text-right text-sm text-gray-600">
                  {months.reduce((sum, month) => sum + monthlyData[month].transactions.length, 0)}
                </td>
              </tr>
            </tfoot>
          </table>
        ...
        {selectedMonth && renderReconciliation(
          selectedMonth, 
  ...
  // Add the renderCategories function before the return statement
  const renderCategories = () => {
    ...
    const handleSaveCategories = () => {
      ...
      // Update all transactions with the new category names
      ...
      <div className="p-6 bg-white rounded-lg shadow">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">Edit category labels and update all transactions</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {uniqueCategories.map(category => (
            <div key={category} className="p-4 border rounded-lg hover:border-blue-300 transition-colors">
  ...
  // Add helper function to get unique categories and sources
  ...
  // Update the renderTransactions function with a total calculation
const renderTransactions = () => {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
        <h3 className="text-lg font-medium">All Transactions ({filteredTransactions.length})</h3>
        <div className="flex flex-wrap gap-4 mt-2 md:mt-0">
          <div className="flex items-center text-sm">
            ...
            <span className={`font-medium ${filteredTotal >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      <div className="mb-4 flex flex-wrap gap-4">
        <div className="flex items-center">
          <label className="mr-2 text-sm font-medium text-gray-700">Category:</label>
          <select
        ...
        <div className="flex items-center">
          <label className="mr-2 text-sm font-medium text-gray-700">Source:</label>
          <select
            value={sourceFilter}
  ...
  // Toggle exclusion for transactions
  ...
  // NEW: Function to update ending balance (now defined inside the component so that setEndingBalances is in scope)
  const handleEndingBalanceChange = (month, value) => {
  ...
  // Function to export transactions and metadata (including endingBalances)
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <div className="mb-6">
        ...
        {/* Use the new file upload handler that also extracts metadata */}
        <input 
          type="file" 
          multiple
          accept=".csv"
        ...
        <div className="border-b border-gray-200 mb-4">
          <nav className="flex -mb-px">
            ...
            <button
              ...
              className={`py-2 px-4 text-sm font-medium ${activeTab === 'current' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Current
            </button>
            <button
            ...
            <button
              ...
              className={`py-2 px-4 text-sm font-medium ${activeTab === 'categories' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Categories
            </button>
            <button
      ...
      {transactions.length > 0 ? (
        ...
        activeTab === 'categories' ? renderCategories() :  // Add this line
